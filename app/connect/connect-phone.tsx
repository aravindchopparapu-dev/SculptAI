...
export default function ConnectPhone({ name, initialCode = '', appState = '' }: { name: string; initialCode?: string; appState?: string }) {
  ...
  const automaticApprovalStarted = useRef(false);
  ...
  const approve = useCallback(async () => {
    ...
    try {
      ...
      setDone(true); setMessage('Account connected. Returning to SculptAI…');
  ...
  useEffect(() => {
    if (!ready || !appState || automaticApprovalStarted.current) return;
    automaticApprovalStarted.current = true;
    void approve();
  }, [ready, appState, approve]);
  return <main style={{ maxWidth: 520, margin: '10vh auto', padding: 28, color: '#eaf5f2', background: '#14222e', borderRadius: 24 }}>
    ...
    {!done && <form onSubmit={event => { event.preventDefault(); void approve(); }}><label hidden={Boolean(appState)} htmlFor="pair-code">iPhone pairing code</label>
      ...
      <button disabled={!ready || busy || code.replace(/[\s-]/g, '').length !== 10} style={{ padding: '14px 24px', borderRadius: 30, background: '#b9f4d8', color: '#12332b' }}>{busy ? 'Finishing sign-in…' : appState ? 'Try returning to SculptAI again' : 'Approve my iPhone'}</button></form>}
