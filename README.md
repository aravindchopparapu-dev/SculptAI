...
The centre offers a private overview, feature switches, exercise availability, a member notice, and full editable instructions for Coach answers, workout plans and meal plans. The current default instructions appear in each editor. Save a draft, test it with fictional profiles, then publish it. The live endpoints read only the published version. Fixed safety and data rules remain in source code. Older published control versions can be loaded into a new draft for review. Preview calls use the server's existing AI key and may consume API credit; no real member data is sent by the preview.
